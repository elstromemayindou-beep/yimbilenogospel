...
const itemVariants = {
  ...
  visible: { 
    ...
    transition: { 
      duration: 0.8, 
      ease: [0.16, 1, 0.3, 1] as const // Correctif TypeScript ajouté ici
    }
  }
};
...
export default function Home() {
  return (
    ...
    >
      {/* Background avec image locale */}
      <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden">
        <img 
          src="/images/artiste.png" 
          alt="Spiritual background" 
          className="w-full h-full object-cover opacity-[0.20] dark:opacity-[0.35] grayscale transition-opacity duration-1000"
        />
      </div>
      ...
      <section className="py-24 md:py-48 px-6 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="flex flex-col lg:flex-row justify-between items-center mb-24 gap-16 text-white">
            ...
            >
              ...
              >
                ...
                <Link to="/reserve" className="btn-primary w-full bg-brand-gold text-white hover:bg-white hover:text-brand-gold transition-all duration-500 py-6 text-xl text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { title: "Concept", desc: "Interprétez un cover dans des conditions professionnelles." },
              { title: "Mission", desc: "Propulser de nouvelles voix gospels d'excellence." },
              { title: "Avantages", desc: "Production de qualité et diffusion sur nos plateformes." }
            ].map((pillar, idx) => (
              <motion.div
                key={idx}
                ...
                viewport={{ once: true }}
                transition={{ delay: idx * 0.2, ease: [0.16, 1, 0.3, 1] as const }}
        ...
        <div className="flex flex-col lg:flex-row items-center gap-16 md:gap-24">
          ...
          >
            ...
            <h2 className="display-title text-4xl md:text-8xl italic text-brand-warm-grey mb-10 leading-tight">
              Vos <span className="text-brand-gold italic">Évènements</span> Clé en Main
            </h2>
            <div className="space-y-8 mb-12">
              <p className="text-lg md:text-2xl text-brand-warm-grey/70 font-light leading-relaxed italic">
                Nous donnons vie à vos visions spirituelles avec excellence.
              </p>
              <p className="text-brand-warm-grey/60 text-base md:text-lg leading-relaxed">
                Yimbiléno Gospel organise vos concerts, séminaires et journées de louange selon votre budget.
              </p>
            </div>
            
            <div className="grid grid-cols-2 gap-6 mb-12">
              {[{ title: "Planification", value: "Sur mesure" }, { title: "Expertise", value: "100% Gospel" }].map((stat, i) => (
          ...
          >
            <div className="relative rounded-[4rem] overflow-hidden shadow-3xl shadow-black/20 group">
              <img 
                src="https://images.unsplash.com/photo-1470229722913-7c0e2dbbafd3?auto=format&fit=crop&q=80" 
                alt="Stage organization" 
      ...
      <section className="py-24 md:py-48 px-6 max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row items-center gap-24">
          <div className="md:w-1/3">
            ...
            <h2 className="display-title text-3xl md:text-8xl italic text-brand-warm-grey mb-12">
          ...
          <div className="md:w-2/3 grid grid-cols-1 gap-12">
            {testimonials.map((t, i) => (
              <motion.div 
                ...
                whileInView={{ opacity: 1, x: 0 }}
                viewport={{ once: true }}
                transition={{ delay: i * 0.2, ease: [0.16, 1, 0.3, 1] as const }}
                className="flex flex-col md:flex-row gap-12 items-center md:items-start p-16 bg-brand-dark rounded-[4rem] border border-white/5 transition-colors duration-500"
              >
                <img src={t.image} alt={t.name} className="w-32 h-32 rounded-full object-cover border-8 border-brand-soft-gold" />
                <div className="flex-grow pt-4 text-center md:text-left">
                  <p className="text-2xl italic text-brand-warm-grey/70 mb-10 font-serif leading-relaxed">"{t.text}"</p>
                  <h5 className="font-display font-black uppercase tracking-[0.3em] text-brand-gold text-[12px]">{t.name}</h5>
                  <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-brand-warm-grey/30 mt-2">{t.role}</span>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      {/* Hyper CTA */}
      <section className="py-32 md:py-64 px-6 relative overflow-hidden text-center bg-brand-warm-grey dark:bg-black border-t border-brand-gold/10 rounded-t-[5rem] md:rounded-t-[10rem]">
        <div className="max-w-5xl mx-auto relative z-10 text-white">
          ...
          >
            ...
            <h2 className="display-title text-4xl md:text-[140px] mb-16 italic leading-[0.85] tracking-tighter text-white">
              Rejoins <br /> la <span className="text-brand-gold italic">Louange</span>
            </h2>
            <Link to="/reserve" className="btn-primary px-12 md:px-24 py-6 md:py-8 text-sm md:text-xl bg-brand-cream text-brand-warm-grey hover:bg-brand-gold hover:text-white inline-block">
